...
**[Pons](https://ponsfamily.com)** — the launchpad on Robinhood Chain — using the **v2 bonding
curve**. Drop your handle, AI drafts your profile coin (name, ticker, bio, avatar, launch thread),
...
- **Handle → profile coin:** paste your fomo.family handle and AI drafts a launch-ready package —
  name, ticker, one-line hook, profile lore, vibe tags, an X launch thread, and an avatar.
- **v2 only, fair launch:** every profile launches on a Pons **v2 bonding curve** that graduates to
...
Frontend (Next.js, FOMO Profile UI)
   → ProfileStudio (handle → AI package → editable launch form)
   → DeployButton → PonsV2Adapter → launchToken() → bonding curve → graduate to Uniswap V4
   → Wallet (wagmi/viem, Robinhood Chain, non-custodial)
```

| Path | Contents |
|---|---|
| `src/lib/pons/` | v2 adapter, registry (verified addresses), ABIs, on-chain readers, curve quote math |
...
| `src/app/api/*` | `generate`, `launches`, `feed`, `upload`, `img`, `v2/*` endpoints |
| `src/app/`, `src/components/` | Landing, launch studio, explore feed, profile-coin page, design system |

The Pons v2 launch engine, wallet setup, and on-chain readers are carried over from the proven Pons
v2 launch app; the profile-centric UI is built on top and the app is intentionally **v2-only**.

## 🚀 Running

```bash
cp .env.example .env.local   # set BANKR_API_KEY (or ANTHROPIC_API_KEY) to enable AI drafting
npm install
npm run dev                  # http://localhost:3000
```

Without an AI key, drafting is disabled — users fill the launch fields by hand and upload an avatar;
...
See [`.env.example`](./.env.example) for the full list: AI provider keys, image storage (Vercel KV /
Upstash), WalletConnect project id, and chain/explorer/contract overrides.

## ⚠️ Notes

- Pons v2 is deployed but **unaudited**, and public launches are **whitelist-gated on-chain**. If a
